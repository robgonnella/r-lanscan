...
pub struct App {
    ...
    selected_view: RefCell<Box<dyn View>>,
}
...
impl App {
    ...
    pub fn new(dispatcher: Arc<dyn Dispatcher>) -> Self {
        Self {
            ...
            selected_view: RefCell::new(Box::new(DevicesView::new(dispatcher))),
        }
    }
    ...
    fn render_footer(
        ...
        if override_legend {
            ...
            let mut info = String::from("(q) quit | (tab) change tab");
    ...
    pub fn next_tab(&self, state: &State) {
        // scoped so borrow is dropped immediately since we need to borrow_mut
        // in next line
        let next = { self.selected_tab.borrow().next() };
        *self.selected_tab.borrow_mut() = next;
        self.set_next_view(state);
    }

    pub fn previous_tab(&self, state: &State) {
        // scoped so borrow is dropped immediately since we need to borrow_mut
        // in next line
        let previous = { self.selected_tab.borrow().previous() };
        *self.selected_tab.borrow_mut() = previous;
        self.set_next_view(state);
    }

    pub fn set_next_view(&self, state: &State) {
        let view: Box<dyn View> = match *self.selected_tab.borrow() {
            SelectedTab::Devices => {
                Box::new(DevicesView::new(Arc::clone(&self.dispatcher)))
            }
            SelectedTab::Config => Box::new(ConfigView::new(
                Arc::clone(&self.dispatcher),
                state.theme,
            )),
            SelectedTab::Logs => Box::new(LogsView::new()),
        };

        *self.selected_view.borrow_mut() = view;
    }
}
...
impl EventHandler for App {
    fn process_event(
        ...
        {
            let view = self.selected_view.borrow();
            let handled = view.process_event(evt, ctx)?;
        ...
        if let CrossTermEvent::Key(key) = evt {
            match key.code {
                KeyCode::Tab => {
                    self.next_tab(ctx.state);
                    return Ok(true);
                }
                KeyCode::BackTab => {
                    self.previous_tab(ctx.state);
                    return Ok(true);
                }
                _ => {}
            }
